...
public class UnicodeToEnglish {
    ...
    public static String codau2khongdau(String input) {
        ...
        input = entities2Unicode(input);
        input = ncrdecimal2unicode(input);
        input = UTF8Literal2Unicode(input);
        input = input.replaceAll("[^–^a-zA-Z0-9:,()+/;&\''_. ^-]","");
        return input;
    }

    public static String codau2khongdauVakitudacbiet(String input){
        if(StringUtil.isNullOrEmpty(input)){
            return "";
        }
        if (codauTokhongdau == null) {
    ...
    public static String UTF8Literal2Unicode(String input) {
        if (StringUtil.isNullOrEmpty(input))
            return "";
        input = input.trim();
        for (int i = 0; i < UTF8Literal.length; i++) {
            input = input.replace(UTF8Literal[i], codau[i]);
        }
